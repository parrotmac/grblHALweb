...
static void driver_delay_ms (uint32_t ms, void (*callback)(void))
{
    if((delay.ms = ms) > 0) {
        ...
        if(!(delay.callback = callback))
            while(delay.ms)
                sim_advance(); // delay.ms is decremented by the simulated SysTick
    } else if(callback)
        callback();
}
...
{
    on &= homing_cycle.mask == 0; // homing polls the switches, a hard limit must not fire
...
static void spindleSetState (spindle_ptrs_t *spindle, spindle_state_t state, float rpm)
{
    ...
    sim_set_spindle(state.on, state.ccw, rpm);
}

// Variable spindle control functions

// Sets spindle speed. There is no PWM peripheral, so map the duty back to the
// RPM it stands for (for the host visualisation only).
static void spindle_set_speed (spindle_ptrs_t *spindle, uint_fast16_t pwm_value)
{
    float rpm = 0.0f;

    if(pwm_value > spindle_pwm.off_value && spindle_pwm.pwm_gradient > 0.0f)
        rpm = spindle_pwm.rpm_min + (float)((int_fast16_t)pwm_value - (int_fast16_t)spindle_pwm.min_value) / spindle_pwm.pwm_gradient;

    spindle_state_t state = { .value = gpio[SPINDLE_PORT].state.value ^ settings.pwm_spindle.invert.mask };
...
{
    return spindle_pwm.compute_value ? spindle_pwm.compute_value(&spindle_pwm, rpm, false) : 0;
}
...
static void coolantSetState (coolant_state_t mode)
{
    ...
    sim_set_coolant(mode.value & COOLANT_MASK);
}
...
// Called on every pass of grbl's main loop and from every wait loop in the core:
// this is where simulated time advances and "interrupts" fire.
static void sim_process_realtime (uint_fast16_t state)
{
    // Main loop heartbeat: the boot sequence (which flushes the input stream) is
    // done and the main loop has run since the last beat. The serial feed is
    // paced against it, mirroring real hardware where the main loop runs many
    // times between two serial bytes.
    sim.grbl_pulse++;
...
*/
    ...
    hal.nvs.get_byte = sim_nvs_get_byte;
    hal.nvs.put_byte = sim_nvs_put_byte;
    hal.nvs.memcpy_to_nvs = sim_nvs_memcpy_to;
    hal.nvs.memcpy_from_nvs = sim_nvs_memcpy_from;
