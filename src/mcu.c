...
void simulate_serial (void)
{
    ...
    // Deliver at most one byte per grbl main-loop pass: a pulse of 0 means grbl is
    // still booting and would flush the input, an unchanged pulse means the main
    // loop has not run since the previous byte - skip this slot, no input is lost.
